...
export default {
  ...
  methods: {
    async getFormList() {
      this.loading = true;
      let ret = await this.API.trees({ data: { node_id: this.cpnt.store.appid, appid: this.cpnt.store.appid, node_dimen: "nodedimen03" } });
      this.options = ret.data.filter(node => node.data_type === "form");
      this.loading = false;
    },
    async getFormCfg() {
      this.loading = true;
