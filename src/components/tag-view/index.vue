...
<script>
import FormTreeForm from "@/components/form/tree-form";
import FormFormForm from "@/components/form/form-form";
import FormListFlow from "@/components/form/list-flow";
export default {
  components: { FormTreeForm, FormFormForm, FormListFlow },
  props: {
    params: {
      type: Object,
      default: () => {
      ...
      if (this.curTag.tag_config) {
        ...
        if (cfg.tag_source) {
          ...
          if (this.cpntParams.where) {
            this.cpntParams.where = { [this.cpntParams.where]: this.curTag.tag_no };
          }
