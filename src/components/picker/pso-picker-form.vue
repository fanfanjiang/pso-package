<template>
  <el-form :label-position="position" :label-width="labelWidth" :size="size">
    <template v-if="!initializing">
      <el-form-item :label="fromText" :required="required">
        <el-select :size="size" :clearable="clearable" v-model="data[formField]" filterable placeholder="工作表" @change="changeHandler">
      ...
      <template v-loading="loading">
        <el-form-item :label="f.n" v-for="(f, i) in fields" :key="i">
          <el-select :size="size" v-model="data[f.f]" filterable :multiple="isArray(data[f.f])" @change="selectHandler($event, f.f)">
